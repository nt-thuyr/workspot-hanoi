    ...
    >
      ...
      >
        ...
        <p className="rhp-modal-desc">
          ...
          <br />
          <span className="rhp-modal-warn">
            キャンセルすると元に戻すことはできません。
          </span>
        </p>
...
const ReservationHistoryPage: FC = () => {
  ...
  const fetchReservationHistory = async () => {
    try {
      ...
      // Lấy lịch sử đặt chỗ
      const response = await fetch(`${API_BASE_URL}/api/reservations/history`, {
  ...
  const executeCancel = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/reservations/${cancelTargetId}/cancel`,
        {
          method: "PATCH",
          headers: {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "キャンセルに失敗しました。", {
          id: toastId,
        });
      }
    } catch (err) {
  ...
  const renderApprovalBadge = (item: ReservationInfo) => {
    ...
    if (appStatus === "REJECTED") {
      return (
        <span
          className="rhp-badge rhp-badge--rejected"
          id={`badge-rejected-${item.id}`}
        >
          拒否済み
        </span>
      );
    ...
    if (appStatus === "APPROVED") {
      return (
        <span
          className="rhp-badge rhp-badge--approved"
          id={`badge-approved-${item.id}`}
        >
          承認済み
        </span>
      );
    ...
    if (appStatus === "PENDING") {
      return (
        <span
          className="rhp-badge rhp-badge--pending"
          id={`badge-pending-${item.id}`}
        >
          未承認
        </span>
      );
    ...
    if (appStatus === "CANCELLED") {
      return (
        <span
          className="rhp-badge rhp-badge--cancelled"
          id={`badge-cancelled-${item.id}`}
        >
          キャンセル済み
        </span>
      );
  ...
  const renderTimeBadge = (item: ReservationInfo) => {
    ...
    if (isUpcoming) {
      return (
        <span
          className="rhp-badge rhp-badge--upcoming"
          id={`badge-upcoming-${item.id}`}
        >
          今後の予定
        </span>
      );
    ...
    return (
      <span className="rhp-badge rhp-badge--past" id={`badge-past-${item.id}`}>
        過去の履歴
      </span>
    );
  };
    ...
    <div className="rhp-root">
      ...
      <div className="rhp-body">
        ...
        <aside className="rhp-sidebar">
          ...
          <div className="rhp-sidebar-section">
            <p className="rhp-sidebar-label" id="sort-label">
              並べ替え
            </p>
        ...
        <main className="rhp-main" id="reservation-main">
          {/* Khu vực 13: Tiêu đề */}
          <h1 className="rhp-page-title" id="page-title">
            予約の履歴
          </h1>
              ...
              {reservedSorted.map((item) => (
                ...
                >
                  ...
                  <div className="rhp-card-body">
                    ...
                    <div
                      className="rhp-card-datetime"
                      id={`card-datetime-${item.id}`}
                    >
                    ...
                    {item.cafeAddress && (
                      <div
                        className="rhp-card-address"
                        id={`card-address-${item.id}`}
                      >
                        <svg
                          width="13"
                          height="13"
                    ...
                    <div
                      className="rhp-card-badges"
                      id={`card-badges-${item.id}`}
                    >
                  ...
                  <div
                    className="rhp-card-right"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {/* Khu vực 17: Mũi tên điều hướng */}
                    <svg
                      className="rhp-card-arrow"
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                      fill="none"
